...
class BoundBox:
    ...
    def get_label(self):
        if self.label == -1:
            self.label = tf.math.argmax(self.classes)
        return self.label
...
def decode_netout(netout, anchors, obj_thresh, net_h, net_w):
    ...
    netout[..., 5:] = netout[..., 4][..., tf.newaxis] * netout[..., 5:]
    ...
    for i in range(grid_h * grid_w):
        ...
        for b in range(nb_box):
            ...
            w = anchors[2 * b + 0] * tf.math.exp(w) / net_w  # unit: image width
            h = anchors[2 * b + 1] * tf.math.exp(h) / net_h  # unit: image height
...
def do_nms(boxes, nms_thresh):
    ...
    for c in range(nb_class):
        sorted_indices = tf.argsort([-box.classes[c] for box in boxes])
